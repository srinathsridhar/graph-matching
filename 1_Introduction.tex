...
recommendations) such that the resulting graph and the website
can be traversed `efficiently.'

In a generalization of the problem, the set of pages can be divided
into a few popular pages where surfers arrive and the remaining large
number of pages that can be potentially recommended from the popular
...
To solve these problems optimally at web-scale graph sizes, implementations would involve
...
solutions with value very close to optimal. We also delineate the parameter
ranges when such a lazy solution would not work. We finally extend the analysis
to more generalized random models of the input graph. 
