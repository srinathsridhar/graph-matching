...
recommendation graph, each vertex $v\in L$ uniformly and independently
...
reality $l$ and $r$ are so much larger than $d$ that repetition is
very unlikely. This model is similar to, but is distinct from the more
...
this model, concentration bounds can show that the degrees of the
vertices in $L$ would have similarly been concentrated around $d$ under
appropriate parameter settings in the Erd\"{o}s-Renyi model. We prove 
...
each $v\in L$ with replacement, the same way the fixed-degree model
...
for our original statement of the algorithm as well.

To prove the claim let $X_{v}$ be the random variable that represents
the degree of the vertex $v$ in our chosen subgraph $H$. Because our
...
edges in $H$ that can be incident on $v$, we can now calculate that
