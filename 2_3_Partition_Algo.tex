...
$b$-matching algorithm (matchings with a specified degree $b$ on each vertex) for any setting of $a$ and $c$. However, implementations of $b$-matching
algorithms often incur significant overheads even
over regular bipartite matchings. This motivates a solution that uses regular bipartite matching algorithms to find an approximately
optimal solution given that a perfect one exists.
...
l}{l}$ and $kc \geq a$ (or $lc \geq ra$), then the probability that $G$ has a perfect $(c, a)$-recommendation
...
%Arda: We assumed kc \geq a, so we always get R' to be all of R??? Can you clarify?
...
%Arda: Is this still true if we have lc >> ar? We might have overcoverage of the nodes of R by more than a sets but this should only help us... At least clarify what is going on in the general case with a few more sentences.
...
%Arda: Add citation to Hopcroft-Karp here
Assuming
