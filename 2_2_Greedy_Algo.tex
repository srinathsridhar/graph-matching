...
successes. We can bound this probability by upper-bounding the $t^{th}$
term above in its binomial expansion by
${{l-\frac{ia}{c}}\choose{t}} (1-p)^{l-\frac{ia}{c}-t}(p)^{t} \leq (lp)^t (1-p)^{l-\frac{ia}{c}-t}$ 
to obtain the following. %$(1-p)^{l-\frac{ia}{c}-a+1}(lp)^{a-1}$ to obtain the following.
...
%I tried to explain where the exponents come from.
...
out sum is $a-1$. Summing over all the $X_i$ using the linearity of
